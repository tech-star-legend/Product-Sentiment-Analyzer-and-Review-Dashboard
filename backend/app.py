...
def search_product():
    ...
    # 🔥 Reduced from 20 → 5 to avoid Render memory crash
    max_reviews = int(data.get("max_reviews", 5))
    ...
    if not scraped["reviews"]:
        return jsonify({
            "error": "No reviews found. Try a different product name."
        }), 404
    ...
    summary = {
        ...
        "avg_score": round(
            sum(r["score"] for r in analyzed_reviews) / total,
            3
        ) if total else 0,
    }
...
def word_frequency(product_id):
    ...
    STOP = {
        ...
        "of", "for", "this", "that", "was", "with", "but", "not",
        "are", "i", "my", "me", "we", "be", "at", "by", "as", "so",
        "if", "do", "he", "she", "they", "you", "have", "has", "had",
        "its", "our", "your", "their", "all", "from", "just", "very",
        "am", "been"
    }
